...
1b. **Hilo garantizado (auditoría 21-jul-2026):** el backend firma toda respuesta con `In-Reply-To`/`References` del mensaje original (helper `hiloHeaders`, DWD readonly) — el hilo se conserva del lado del cliente en cualquier cliente de correo, sin importar desde qué cuenta salga. Si el agente envía respuestas directamente por el MCP de Gmail, DEBE pasar `thread_id`, `in_reply_to` y `references` del mensaje original en `send_gmail` — nunca responder un hilo con un correo suelto.
...
3. **Copias en correos NUEVOS:** además de la regla anterior, todo correo nuevo lleva en CC al **owner asignado del cliente** (columna `owner` de la pestaña Clientes; mapa nombre→email en `Config.owners_emails`, defaults: Eduardo→eduardo@, **Cristian→cristian@**, Edgar→edgar.martinez@, Arturo→arturo@). ⚠️ Cristina salió del portafolio el 10-ago-2026: su cartera es de Cristian. Si una fila legacy trae `owner=Cristina`, el CC va a **cristian@tally.legal** (nunca a cristina@)..
...
> **Estado `Respuesta OS` (28-jul-2026): ⛔ EL AGENTE NO REDACTA NI ENVÍA sobre estas filas.** Significa "respuesta Out Side": el backend detectó (vía DWD readonly, acción `detectar_os` que corre dentro de `sync_inbox`) que alguien del equipo YA respondió el hilo por fuera de la interfaz — quién/cuándo/snippet quedan en la columna X `respuesta_os`. La detección tiene DOS fases: (1) el hilo de la cuenta que recibió el correo, y (2) barrido cruzado de los enviados recientes de TODAS las cuentas de `Config.cuentas_monitoreadas` (match por destinatario del cliente + asunto normalizado), para atrapar respuestas hechas desde otra cuenta Tally. El agente tampoco marca este estado por su cuenta: lo pone solo el backend. Si Juan da una nueva instrucción sobre una fila OS desde la interfaz, el flujo normal la reactiva (Prompt recibido) y ahí sí se procesa.
