...
**Dueño del proceso:** Contabilidad (owner del cliente) · **Responsable del SOP:** Juan (CSO) · **Sistema:** Tally Ops → sección "Cálculo de impuestos" · **Versión:** 1.3 · 4-sep-2026
...
| Facturas de gastos a nombre del cliente (CFDI de ingreso donde su RFC es el receptor) | El cliente las pide a sus proveedores | antes del día 10 | el IVA acreditable del mes queda en cero y el documento lo advierte |
...
- **Estados de cuenta:** se reconocen tres familias, en este orden.
  1. **Los que declaran su propio resumen** (Wells Fargo, Chase, BBVA, Banorte, Santander y similares): se leen los cuatro números que el banco publica —saldo inicial, depósitos/abonos, retiros/cargos y saldo final— y se cuadran entre sí. No se suman renglones: el banco ya sumó. Además se identifican por concepto los **depósitos que vienen del marketplace** y se muestran aparte del total, porque son los comparables contra lo que reportó el marketplace.
  2. **Tabla *fecha · concepto · monto · saldo*** (WorldFirst y cualquier banco con esa forma): el signo de cada movimiento **no se adivina por el concepto**, se deduce del salto del saldo renglón por renglón.
  3. **Payoneer** (renglones "Payment from …").
  En los tres casos la tabla del paso 2 marca *cuadra* o *no cuadra* (`saldo inicial + abonos − cargos = saldo final`); si no cuadra, no continúes sin revisar el PDF. Si el estado de cuenta **no está en pesos** (por ejemplo la cuenta en dólares de una LLC), el documento lo advierte: esos abonos no son comparables peso a peso con las transferencias del marketplace en México.
