...
## Retenciones, gastos y estados de cuenta (4-sep-2026, v9–v11)
- `calcRetBloque_` + `calcParseCertificado_`: los importes del certificado salen del bloque **Impuestos retenidos** (renglón de IVA: base = IVA trasladado, importe = IVA retenido; renglón de ISR: base = monto de operación, importe = ISR retenido). El tipo de impuesto se deduce del tipo de pago cuando no viene en el mismo renglón. Los `Total …` del pie quedan como respaldo porque en certificados grandes las cifras se parten en dos renglones.
- Motor (las **dos** copias, `index.html` y `scripts/apps-script.gs`): el IVA retenido lo manda el certificado; la liquidación del marketplace es cotejo y levanta alerta si difiere. El residuo de dividir entre 0.16 se absorbe en la base 16%.
- `calcSyntage_`: clasificación emisor/receptor por `isIssuer`/`isReceiver` con respaldo por RFC (hay entidades sin RFC en el catálogo de Syntage); la consulta no filtra `status` en el servidor (se filtra a VIGENTE aquí) y si la consulta con filtro de fecha viene vacía se reintenta sin filtro y se recorta al ejercicio. Devuelve `cfdi_del_anio`, `cfdi_emitidos_anio` y `cfdi_recibidos_anio` para que el paso 3 muestre si el SAT reporta o no facturas recibidas.
- La lista de CFDI recibidos que va a la hoja de gastos se limita a tipos I y E; cada bloque de CFDI del Excel cierra con renglón de totales.
- `calcParseResumenBanco_`: primera ruta para estados de cuenta. Lee el resumen que declara el banco (saldo inicial, depósitos, retiros, saldo final), lo cuadra, identifica por concepto los depósitos del marketplace, y detecta banco, últimos cuatro dígitos, moneda y período. Probado con Wells Fargo Business Checking de 3J TRADING LLC julio-2026 (cuadra exacto; 8 depósitos de Amazon = todo el abono del mes).
- Lector de PDF: cadena de filtros (`ASCII85Decode`, `FlateDecode`, `ASCIIHexDecode`), CTM de `q`/`Q`/`cm` —sin esto los PDF que colocan cada bloque de texto con `cm` colapsaban en una sola línea— y tope de 15 páginas.
